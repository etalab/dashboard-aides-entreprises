...
psql -d dashboard -c "\copy (SELECT * FROM (SELECT 'Prêts garantis par l''Etat' as dispositif, sum(P.nombre) as nombre_pge, sum(P.montant) as montant_total, P.reg, REG.libelle as libelle_region, P.code_section, N.libelle_section, MAX(P.last_update) as date_maj FROM pgenaf P LEFT JOIN region REG ON REG.reg = P.reg LEFT JOIN (SELECT DISTINCT code_section, libelle_section FROM naf) N ON P.code_section = N.code_section GROUP BY P.reg, REG.libelle, P.code_section, N.libelle_section ORDER BY REG.libelle, P.code_section) tbl WHERE nombre_pge > 2) TO '/tmp/pge-regional-naf-latest.csv' DELIMITER ',' CSV HEADER;"
mv /tmp/pge-regional-naf-latest.csv ../published-data/

psql -d dashboard -c "\copy (SELECT * FROM (SELECT 'Prêts garantis par l''Etat' as dispositif, sum(P.nombre) as nombre_pge, sum(P.montant) as montant_total, P.reg, REG.libelle as libelle_region, P.dep, D.libelle as libelle_departement, P.code_section, N.libelle_section, MAX(P.last_update) as date_maj FROM pgenaf P LEFT JOIN region REG ON REG.reg = P.reg LEFT JOIN departement D ON D.dep = P.dep LEFT JOIN (SELECT DISTINCT code_section, libelle_section FROM naf) N ON P.code_section = N.code_section GROUP BY P.reg, REG.libelle, P.dep, D.libelle, P.code_section, N.libelle_section ORDER BY REG.libelle, D.libelle, P.code_section) tbl WHERE nombre_pge > 2) TO '/tmp/pge-departemental-naf-latest.csv' DELIMITER ',' CSV HEADER;"
